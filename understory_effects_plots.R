...
labels <- c("Tree cover", "Mean change in tree canopy (%)")

tiff(filename="./outputs/Figure_4_all_understory_effects.tiff", 
     type="cairo",
     units="in", 
     width = 3, 
     height=5, 
...
cg_change_10_no_out <- preds_cg_change(cg_change_lm_no_outliers, C_cg_change_10, "Delta_pdc")
cg_change_50_no_out <- preds_cg_change(cg_change_lm_no_outliers, C_cg_change_50, "Delta_pdc")
cg_change_90_no_out <- preds_cg_change(cg_change_lm_no_outliers, C_cg_change_90, "Delta_pdc")
...
#generate the figure
tiff(filename="./outputs/Figure_3_cheatgrass_change_effect.tif", 
    type="cairo",
    units="in", 
    width = 4, 
    height=3, 
...
par(mar = c(0,1,0,1), oma = c(4,4,2,2), bty = 'n')
...
for(i in c(1:3)){
  ...
  # I also hate this code: plot the residuals ~ delta_tc for each level of CWD
      if(i == 1){
        points(compare2[compare2$cwd_normal_cum < unname(quantile(compare2$cwd_normal_cum, 0.3)),]$delta_cg/100 ~ 
          compare2[compare2$cwd_normal_cum < unname(quantile(compare2$cwd_normal_cum, 0.3)),]$Delta_pdc,
          bg='grey60',
          col = 'grey30',
          pch = 21,
          cex = 1.2)
        ...
        points(compare2[findInterval(compare2$cwd_normal_cum, unname(quantile(compare2$cwd_normal_cum, c(0.3, 0.7)))) == 1,]$delta_cg/100 ~ 
          compare2[findInterval(compare2$cwd_normal_cum, unname(quantile(compare2$cwd_normal_cum, c(0.3, 0.7)))) == 1,]$Delta_pdc,
          bg='grey60',
          col = 'grey30',
          pch = 21,
          cex = 1.2)
        points(compare_outliers[1, ]$delta_cg/100 ~ compare_outliers[1, ]$Delta_pdc, 
               bg='white',
               col = 'grey30',
               pch = 21,
               cex = 1.2)
        ...
        points(compare2[compare2$cwd_normal_cum > unname(quantile(compare2$cwd_normal_cum, 0.7)),]$delta_cg/100 ~ 
          compare2[compare2$cwd_normal_cum > unname(quantile(compare2$cwd_normal_cum, 0.7)),]$Delta_pdc,
          bg='grey60',
          col = 'grey30',
          pch = 21,
          cex = 1.2)
        points(compare_outliers[2, ]$delta_cg/100 ~ compare_outliers[2, ]$Delta_pdc, 
               bg='white',
               col = 'grey30',
               pch = 21,
               cex = 1.2)
 ...
 axis(side = 1, at = c(-60, -40, -20, 0, 10), labels = TRUE)
 axis(side = 1, at = c(-50, -30, -10), labels = FALSE, tcl = -.25)
 text(x = -30, y = .14, labels = panel_labels[i], cex = 1)

}
   
  mtext(side = 1, text = "Mean change in live canopy (%)", outer = TRUE, line = 2.6)
...
tiff(filename="./outputs/Supplementary_figure_effects_pgrass.tiff", 
     type="cairo",
     units="in", 
     width = 7, 
     height=5, 
...
for(i in 1:2){
  ...
  if(i == 1){
    x_new <- eff[["x"]][[var]] * 100
    x_orig <- eff[["x.all"]][[var]] * 100
    line_data$x <- line_data$x * 100
  }
...
#three CWD:delta_pdc panels
for(i in 1:3){
  
  eff <- Effect(model, partial.residuals = TRUE, focal.predictors = c("Delta_pdc", "cwd_normal_cum"),
                xlevels = list(Delta_pdc = 100),
  ...
  line_data <- data.frame(y = inv.as(eff$fit[1:99 + (i-1) * 100]),
                          ...
                          x = eff[["x"]][["Delta_pdc"]][1:99 + (i-1) * 100]
  )
  
  x_new <- eff[["x"]][["Delta_pdc"]]
  x_orig_all <- eff[["x.all"]]
  which_x <- x_orig_all$cwd_normal_cum - cwd_levels[i] < 0.001
  x_orig <- x_orig_all[which_x, ][["Delta_pdc"]]
...
mtext(side = 1, text = "Mean change in live canopy (%)", outer = TRUE, line = 0)
...
# cheatgrass
###############################################################################
tiff(filename="./outputs/Supplementary_figure_effects_cheatgrass.tiff", 
     type="cairo",
     units="in", 
     width = 7, 
     height=5, 
...
# shrub
###############################################################################
tiff(filename="./outputs/Supplementary_figure_effects_shrub.tiff", 
     type="cairo",
     units="in", 
     width = 7, 
     height=5, 
...
# perennial forbs
###############################################################################
tiff(filename="./outputs/Supplementary_figure_effects_pforb.tiff", 
     type="cairo",
     units="in", 
     width = 7, 
     height=5, 
