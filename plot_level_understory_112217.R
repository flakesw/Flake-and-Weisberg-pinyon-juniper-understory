...
daub <- read.csv("daub_cover.csv")
  ...
  daub[daub$Plot == "SS2556120", "Plot"] <- "SS256120"
...
plot_data <- join(plot_data, clim[, c("Plot", "cwd_normal_cum", "avg_ann_ppt")], by = "Plot")
plot_data <- join(plot_data, soil[, c("Plot", "AWC")], by = "Plot")
plot_data <- join(plot_data, tcover[, c("Plot", "Tree_cover", "Shrub_cover_li")], by = "Plot")
plot_data <- join(plot_data, other_vars[, c("Plot", "Cluster", "Avg_depth", "Elev", "Slope", "Swness")], by = "Plot")
...
# Ordination analysis
#----------------------------------------
raw_cover <- read.csv("spp_cover2.csv") #spp_cover2 has unknowns filled with "unknown"
raw_cover <- raw_cover[raw_cover$Transect %in% c("E", "N", "S", "W"), ] #take out the "bonus" quadrats

spp_summary <- read.csv("species_summary.csv")
...
env_data <- plot_data[, c("cwd_normal_cum", "Delta_pdc", "AWC", "Live_ba", 
                          "Dead_ba", "Avg_depth", "Elev", "Slope", "Swness", "avg_ann_ppt")]
