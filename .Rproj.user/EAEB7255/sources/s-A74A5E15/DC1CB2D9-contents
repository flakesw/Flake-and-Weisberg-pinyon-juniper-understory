...
soil <- calculate_awc("./Raw data/soils_missing_imputed_012016.csv")
clim <- read.csv("./Raw data/ALL_climate_variables.csv")
names(clim)[2] <- "Plot"
other_vars <- read.csv("./Raw data/all_vars_EXPORT.csv")
map_mat <- read.csv("./Raw data/map_mat_normals.csv")
names(map_mat)[2] <- "Plot"
...
plot_data <- join(plot_data, map_mat[, c("Plot", "tmean", "ppt")], by = "Plot")
